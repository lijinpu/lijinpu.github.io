...
export const getStaticProps: GetStaticProps<Props> = async () => {
  // 指向当前页面文件（如果文件名/路径不同，请对应修改）
  const filePath = path.join(process.cwd(), 'pages', 'cv.tsx')
  const mtime = fs.statSync(filePath).mtime.toISOString()

  return {
    props: { lastUpdateISO: mtime },
    // ISR：最多每 60 秒再生一次（可按需改）
    revalidate: 60,
  }
}
    ...
    <div className="flex flex-col relative max-w-none prose dark:prose-invert">
      ...
      <div>
        ...
        <ul>
          ...
          <li><a href="https://ssrn.com/abstract=4790765">Asset Bubbles in the Data Economy</a>, with Feng Dong, 2024.</li>
          <li><a href="https://ssrn.com/abstract=4664864">Optimal Data Pricing Decisions of Competitive Two-sided Platforms with Heterogeneous Data Costs</a>, with Ronghua Sui and Ruize Gao, 2024.</li>
        </ul>
        ...
        <ul>
          <li>2025: INFORMS 2025 Annual Meeting, The 2025 Modern Risk Society International Risk Conference (MRS2025); 《经济学（季刊）》“人工智能的经济影响：前沿研究与政策启示”专题研讨会, 清华大学第788期博士生学术论坛</li>
          <li>2024: Asia Meeting of the Econometric Society East & Southeast Asia (AMES), China Financial Research Conference (CFRC), Asian Finance Association Annual Conference (AsianFA), CCER Summer Institute (CCERSI), SWUFE International Macro-Finance Conference, The Consumer Theme Conference “Navigating Chinese Economy: A Demand Perspective”; 第二届“中国数字经济发展论坛”, 第六届“中国发展经济学前沿学术会议”, 清华大学第743期博士生学术论坛</li>
        </ul>
      ...
      {/* Last update 放在页面底部 */}
      <footer className="mt-8 text-gray-400 text-sm text-right">
        Last update: {en}（{zh}）
      </footer>
    </div>
  )
